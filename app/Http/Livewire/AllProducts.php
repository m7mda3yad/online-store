...
class AllProducts extends Component{
    ...
    public function updatedFilterId()
    {
        if($this->filterId==null){
            ...
            if($this->my_price!=0)
            $this->products  = $this->sub_category->products()->where('real_price', '<=' , $this->my_price)->WhereIn('id',$ids)->get();
            else
            $this->products  = $this->sub_category->products()->WhereIn('id',$ids)->get();
        }
    }
    public function filte($id,$type)
    {
        // if($this->filters==1)
        $this->filters[]+=$id;
        // else
        // if (($key = array_search($id, $this->filters)) !== false) {
