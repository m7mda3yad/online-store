      ...
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>{{ trans('cruds.categories') }}</h1>
            <br>
            <div class="col-sm-4">
                @canany(['admin', 'add categories'])
                    <button type="button" class="btn btn-block btn-primary " data-toggle="modal" data-target="#modal-lg">{{ trans('cruds.add') }} {{trans('cruds.categories') }}</button>
                @endcan
            </div>
                <div class="col-sm-12">
                @if ($errors->any())
                        ...
                        <td>
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu" role="menu">
                                    ...
                                    @canany(['admin', 'update categories'])
                                    <a class="dropdown-item" href="{{route('categories.edit',$item->id)}}" >{{ trans('cruds.edit') }}</a>
                                    @endcan

                                    @canany(['admin', 'suspend categories'])
                                        <a class="dropdown-item" href="{{route('categories.suspend',$item->id)}}" >
                                            {{ $item->active==1?trans('cruds.suspend'):trans('cruds.unsuspend') }}</a>
                                    @endcan

                                    @canany(['admin', 'delete categories'])
