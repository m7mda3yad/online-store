      ...
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>{{ trans('cruds.countries') }}</h1>
            <br>
            <div class="col-sm-4">
                @canany(['admin', 'add countries'])
                    <button type="button" class="btn btn-block btn-primary " data-toggle="modal" data-target="#modal-lg">{{ trans('cruds.add') }} {{trans('cruds.countries') }}</button>
                @endcan
            </div>
                <div class="col-sm-12">
                @if ($errors->any())
                            ...
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu" role="menu">
                                    @canany(['admin', 'view all countries'])
                                        <a class="dropdown-item" href="{{route('countries.show',$item->id)}}" >{{ trans('cruds.view') }}</a>
                                    @endcan

                                    @canany(['admin', 'update countries'])
                                    <a class="dropdown-item" href="{{route('countries.edit',$item->id)}}" >{{ trans('cruds.edit') }}</a>
                                    @endcan

                                    @canany(['admin', 'suspend countries'])
                                        <a class="dropdown-item" href="{{route('countries.suspend',$item->id)}}" >
                                            {{ $item->active==1?trans('cruds.suspend'):trans('cruds.unsuspend') }}</a>
                                    @endcan

                                    @canany(['admin', 'delete countries'])
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="card card-primary">
                    ...
                    <h3 class="card-title">{{ trans('cruds.add') }} {{trans('cruds.countries') }}</h3>
                    </div>
