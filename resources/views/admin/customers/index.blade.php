      ...
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>{{ trans('cruds.customers') }}</h1>
            <br>
            <div class="col-sm-4">
                @canany(['admin', 'add customers'])
                    <button type="button" class="btn btn-block btn-primary " data-toggle="modal" data-target="#modal-lg">{{ trans('cruds.add') }} {{trans('cruds.customers') }}</button>
                @endcan
            </div>
                <div class="col-sm-12">
                @if ($errors->any())
                            ...
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu" role="menu">
                                    @canany(['admin', 'view all customers'])
                                        <a class="dropdown-item" href="{{route('customers.show',$item->id)}}" >{{ trans('cruds.view') }}</a>
                                    @endcan

                                    @canany(['admin', 'update customers'])
                                    <a class="dropdown-item" href="{{route('customers.edit',$item->id)}}" >{{ trans('cruds.edit') }}</a>
                                    @endcan

                                    @canany(['admin', 'suspend customers'])
                                        <a class="dropdown-item" href="#" >
                                            {{ $item->active==1?trans('cruds.suspend'):trans('cruds.unsuspend') }}</a>
                                    @endcan

                                    @canany(['admin', 'delete customers'])
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="card card-primary">
                    ...
                    <h3 class="card-title">{{ trans('cruds.add') }} {{trans('cruds.customers') }}</h3>
                    </div>
