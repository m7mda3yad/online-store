      ...
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>{{ trans('cruds.sub_filters') }}</h1>
            <br>
            <div class="col-sm-4">
                @canany(['admin', 'add sub_filters'])
                    <button type="button" class="btn btn-block btn-primary " data-toggle="modal" data-target="#modal-lg">{{ trans('cruds.add') }} {{trans('cruds.sub_filters') }}</button>
                @endcan
            </div>
                <div class="col-sm-12">
                @if ($errors->any())
                            ...
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu" role="menu">
                                    @canany(['admin', 'view all sub_filters'])
                                        <a class="dropdown-item" href="{{route('sub_filters.show',$item->id)}}" >{{ trans('cruds.view') }}</a>
                                    @endcan

                                    @canany(['admin', 'update sub_filters'])
                                    <a class="dropdown-item" href="{{route('sub_filters.edit',$item->id)}}" >{{ trans('cruds.edit') }}</a>
                                    @endcan

                                    @canany(['admin', 'suspend sub_filters'])
                                        <a class="dropdown-item" href="{{route('sub_filters.suspend',$item->id)}}" >
                                            {{ $item->active==1?trans('cruds.suspend'):trans('cruds.unsuspend') }}</a>
                                    @endcan

                                    @canany(['admin', 'delete sub_filters'])
                                    <form method="POST" action="{{ route('sub_filters.destroy', $item->id) }}"
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="card card-primary">
                    ...
                    <h3 class="card-title">{{ trans('cruds.add') }} {{trans('cruds.sub_filters') }}</h3>
                    </div>
                    <form action="{{route('sub_filters.store')}}" method="POST" enctype="multipart/form-data" >
                        @csrf
                        <div class="card-body">
                        ...
                        <div class="col-6">
                            <label >{{trans('cruds.filter')}}</label>
