      ...
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>{{ trans('cruds.sub_categories') }}</h1>
            <br>
            <div class="col-sm-4">
                @canany(['admin', 'add sub_categories'])
                    <button type="button" class="btn btn-block btn-primary " data-toggle="modal" data-target="#modal-lg">{{ trans('cruds.add') }} {{trans('cruds.sub_categories') }}</button>
                @endcan
            </div>
                <div class="col-sm-12">
                @if ($errors->any())
                            ...
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu" role="menu">
                                    @canany(['admin', 'view all sub_categories'])
                                        <a class="dropdown-item" href="{{route('sub_categories.show',$item->id)}}" >{{ trans('cruds.view') }}</a>
                                    @endcan

                                    @canany(['admin', 'update sub_categories'])
                                    <a class="dropdown-item" href="{{route('sub_categories.edit',$item->id)}}" >{{ trans('cruds.edit') }}</a>
                                    @endcan

                                    @canany(['admin', 'suspend sub_categories'])
                                        <a class="dropdown-item" href="{{route('sub_categories.suspend',$item->id)}}" >
                                            {{ $item->active==1?trans('cruds.suspend'):trans('cruds.unsuspend') }}</a>
                                    @endcan
                                    @canany(['admin', 'assign filter'])
                                        <a class="dropdown-item" href="{{route('sub_categories.assign_filter',$item->id)}}" >
                                            {{trans('cruds.filter') }}</a>
                                    @endcan

                                    @canany(['admin', 'delete sub_categories'])
                                    <form method="POST" action="{{ route('sub_categories.destroy', $item->id) }}"
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="card card-primary">
                    ...
                    <h3 class="card-title">{{ trans('cruds.add') }} {{trans('cruds.sub_categories') }}</h3>
                    </div>
